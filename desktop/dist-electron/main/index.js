import { ipcMain, app, BrowserWindow, Tray, nativeImage, Menu } from "electron";
...
class ProductivityTracker {
  ...
  stop() {
    ...
    if (this.currentWindow && this.windowStartTime) {
      const record = this.createRecord(this.currentWindow, this.windowStartTime, /* @__PURE__ */ new Date());
  ...
  trackWindow() {
    if (!activeWin) return;
    activeWin().then((window2) => {
      if (!window2) return;
      if (!this.currentWindow || this.currentWindow.title !== window2.title) {
        if (this.currentWindow && this.windowStartTime) {
          const record = this.createRecord(this.currentWindow, this.windowStartTime, /* @__PURE__ */ new Date());
          this.logRecord(record);
        ...
        this.windowStartTime = /* @__PURE__ */ new Date();
      }
    }).catch((error) => {
   ...
   */
  createRecord(window2, startTime, endTime) {
    const durationSeconds = Math.floor((endTime.getTime() - startTime.getTime()) / 1e3);
    let inputActivity = void 0;
    if (PLATFORM === "win32") {
      try {
        const { getInputMonitor } = require2("./input-monitor");
    ...
    return {
      app_name: window2.owner.name,
      window_title: window2.title,
      category: this.categorizeApp(window2.owner.name, window2.title),
   ...
   */
  categorizeApp(appName, windowTitle) {
    const lowerAppName = appName.toLowerCase();
    const lowerTitle = (windowTitle || "").toLowerCase();
    if (lowerAppName.includes("vscode") || lowerAppName.includes("intellij") || lowerAppName.includes("pycharm") || lowerAppName.includes("webstorm") || lowerAppName.includes("xcode") || lowerAppName.includes("android studio") || lowerTitle.includes("visual studio")) {
      return "work";
    }
    if (lowerTitle.includes("documentation") || lowerTitle.includes("stack overflow") || lowerTitle.includes("github") || lowerTitle.includes("gitlab") || lowerAppName.includes("safari") || lowerAppName.includes("chrome") && (lowerTitle.includes("wikipedia") || lowerTitle.includes("docs."))) {
      return "study";
    }
    if (lowerAppName.includes("slack") || lowerAppName.includes("teams") || lowerAppName.includes("discord") || lowerAppName.includes("zoom") || lowerAppName.includes("skype")) {
      return "communication";
    }
    if (lowerAppName.includes("youtube") || lowerAppName.includes("netflix") || lowerAppName.includes("spotify") || lowerAppName.includes("games") || lowerAppName.includes("steam")) {
      return "leisure";
    }
    return "work";
  }
...
class SyncService {
  ...
  async sync() {
    ...
    if (maxRetryCount > 0) {
      console.log(`Backoff: waiting ${backoffDelay / 1e3}s before retry attempt ${maxRetryCount + 1}`);
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        this.queue = this.queue.filter(
          (item) => !retryableItems.some((retryable) => retryable.id === item.id)
        );
        if (this.retryTimeout) {
          clearTimeout(this.retryTimeout);
          this.retryTimeout = null;
   ...
   */
  getQueueStatus() {
    const lastError = this.queue.length > 0 ? this.queue.find((i) => i.lastError)?.lastError || null : null;
    return {
...
function createTray() {
  if (platform === "darwin" || platform === "win32") {
    tray = new Tray(
      nativeImage.createFromPath(path$1.join(__dirname, "../../../build/icon.png")).resize({ width: 16, height: 16 })
    );
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    height: 600,
    show: false,
    // Don't show initially, only show in tray
...
app.whenReady().then(() => {
  ...
  tracker.start(5);
  if (platform === "win32") {
    import("./chunks/input-monitor-DMa8LeHa.js").then(({ getInputMonitor }) => {
